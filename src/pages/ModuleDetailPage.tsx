...
export function ModuleDetailPage({ course, module, storage, onBack, onBackToCourse }: ModuleDetailPageProps) {
  const { addWord, updateWord, deleteWord, settings } = storage;
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [editingWord, setEditingWord] = useState<Word | null>(null);

  const handleCreateWord = (wordData: Omit<Word, 'id'>) => {
    addWord(course.id, module.id, wordData);
    setShowCreateModal(false);
  };

  const handleEditWord = (wordData: Omit<Word, 'id'>) => {
    if (!editingWord) return;
    updateWord(course.id, module.id, editingWord.id, wordData);
    setEditingWord(null);
  };

  const handleDeleteWord = (wordId: string) => {
    deleteWord(course.id, module.id, wordId);
  };
